...
class AndroidCodexRuntime(
    ...
    override suspend fun start() {
        ...
        try {
            if (runtimeOverride == null) verifyPackagedRuntime(executable)
    ...
    private fun watch(current: Process) {
        scope.launch {
            try {
                readStrictJsonLines(current.inputStream) { line ->
    ...
    private fun verifyPackagedRuntime(executable: File) {
        ...
        distribution.requireCompatible(
            ...
            RuntimeEnvironment(
                ...
                supportsStaticElf = "arm64-v8a" in Build.SUPPORTED_ABIS,
            ),
        )
        check(executable.isFile && executable.sha256() == distribution.binarySha256) {
