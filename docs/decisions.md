...
| Use Codex-managed ChatGPT browser authentication inside the user-started foreground session | One tap opens the official sign-in URL while app-server owns the local callback, token persistence, and refresh; the service remains the one runtime owner while the browser is foregrounded | The provider supplies an Android-native sign-in flow |
