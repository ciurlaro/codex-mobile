...
- **Remote verification:** Draft PR #1 runs only commit-pinned actions. GitHub Actions Verify run 11 passed both the structure job and the complete signed build/lint/release-verification/reproducibility job after resolving the published API 37.0 package name and adding the exact Google-hosted Linux AAPT2 checksum to strict dependency metadata.
