# Third-party notices

Codex Mobile packages Codex App Server `0.145.0` (Apache-2.0), Kotlin and
kotlinx libraries (Apache-2.0), AndroidX (Apache-2.0), Okio (Apache-2.0), and
their locked runtime dependencies. Exact versions and hashes are recorded in
`docs/technical/sbom.cdx.json`.

Chat math is rendered locally by RaTeX `0.1.13` and its bundled KaTeX fonts
(MIT). No WebView or remote math service is used.
