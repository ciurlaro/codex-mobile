...
output="$root/build/modules/android/app/outputs"
apk=${1:-"$output/apk/release/app-release.apk"}
aab=${2:-"$output/bundle/release/app-release.aab"}
...
test -s "$output/mapping/release/mapping.txt"
...
for lock in \
  ...
  modules/tooling/build-logic/settings-gradle.lockfile; do
  test -s "$root/$lock"
done
