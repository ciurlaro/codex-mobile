...
build() {
    "$root/gradlew" --no-build-cache clean :android:app:assembleRelease
}
...
if [[ -n "$baseline" ]]; then
    ...
    build
    cp "$root/build/modules/android/app/outputs/apk/release/app-release.apk" "$evidence/first.apk"
fi
build
cp "$root/build/modules/android/app/outputs/apk/release/app-release.apk" "$evidence/second.apk"
