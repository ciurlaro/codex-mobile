...
abstract class AppReadinessDeviceTestBase {
    ...
    protected fun findNode(text: String): AccessibilityNodeInfo {
        waitForNode(text)?.let { return it }
        for (action in intArrayOf(
            ...
            for (attempt in 0 until 100) {
                ...
                nodes.firstOrNull { nodeLabel(it) == text }?.let { return it }
                val scrollable = nodes.firstOrNull { it.isScrollable }
                if (scrollable?.performAction(action) != true) break
                instrumentation.waitForIdleSync()
            }
        }
        waitForNode(text)?.let { return it }
        val activeRoot = root()
        val activeNodes = flatten(activeRoot)
        throw AssertionError(
            "Accessibility node not found: $text; package=${activeRoot.packageName}; " +
                "class=${activeRoot.className}; nodes=${activeNodes.size}; " +
                "labels=${activeNodes.mapNotNull(::nodeLabel)}",
        )
    }

    private fun waitForNode(text: String): AccessibilityNodeInfo? {
        repeat(20) {
            flatten(root()).firstOrNull { nodeLabel(it) == text }?.let { return it }
        ...
        repeat(50) {
            ...
            shell(
                "input swipe ${metrics.widthPixels / 2} ${metrics.heightPixels * 3 / 4} " +
                    "${metrics.widthPixels / 2} ${metrics.heightPixels / 2} 250",
            )
            instrumentation.waitForIdleSync()
        }
    ...
    private fun buttonAncestor(start: AccessibilityNodeInfo): AccessibilityNodeInfo? {
