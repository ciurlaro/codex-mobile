...
@Composable
internal fun MainContent(activity: MainActivity, viewModel: AppViewModel) {
    ...
    val signInAuthentication = rememberLauncherForActivityResult(
        ...
        if (result.resultCode != Activity.RESULT_OK && !state.isAuthenticated) viewModel.browserUnavailable()
    }
    fun openSignIn(rawUrl: String?) {
        ...
        if (signInUri == null) {
            ...
            signInAuthentication.launch(ConnectorAuthActivity.intent(activity, signInUri.toString()))
        }
    }
    ...
    val elicitationAuthentication = rememberLauncherForActivityResult(
        ...
        if (result.resultCode != Activity.RESULT_OK) state.pendingElicitation?.let { elicitation ->
            viewModel.resolveElicitation(
                elicitation.requestId,
                io.github.ciurlaro.codexmobile.agent.AgentElicitationResponse(
                    io.github.ciurlaro.codexmobile.agent.AgentElicitationAction.CANCEL,
                ),
            )
        }
    }
    ...
    LaunchedEffect(state.connectorAuthUrl) {
        ...
        if (url != null && url != openedConnectorUrl) {
            ...
            connectorAuthentication.launch(
                ConnectorAuthActivity.intent(activity, url),
            )
        }
    }
    ...
    AppTheme {
        ...
        when {
            codexApproval != null -> CodexApprovalDialog(
                codexApproval,
                String::toApprovalDisplayText,
            ) { decision ->
