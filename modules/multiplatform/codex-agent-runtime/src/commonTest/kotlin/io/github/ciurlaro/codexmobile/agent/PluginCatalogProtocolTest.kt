...
class PluginCatalogProtocolTest : SkillsPluginsProtocolTestBase() {
    ...
    fun availablePluginDiscoveryServesCacheAndKeepsStaleDataAfterRefreshFailure(): Unit = runBlocking {
        ...
        CodexAgentClient({ process }, requestTimeoutMillis = 1_000, pluginCacheDirectory = Path(cache.absolutePath)).use { client ->
    ...
    fun installedPluginRefreshKeepsLastKnownPluginsWhenAppServerReturnsEmpty(): Unit = runBlocking {
        ...
        CodexAgentClient({ populated }, requestTimeoutMillis = 1_000, pluginCacheDirectory = Path(cache.absolutePath)).use { client ->
        ...
        CodexAgentClient({ empty }, requestTimeoutMillis = 1_000, pluginCacheDirectory = Path(cache.absolutePath)).use { client ->
