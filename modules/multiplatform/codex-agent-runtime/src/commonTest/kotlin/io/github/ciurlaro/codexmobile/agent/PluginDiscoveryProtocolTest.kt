...
class PluginDiscoveryProtocolTest : SkillsPluginsProtocolTestBase() {
    ...
    fun signOutClearsSavedPluginCatalogs(): Unit = runBlocking {
        ...
        CodexAgentClient({ runtime }, requestTimeoutMillis = 1_000, pluginCacheDirectory = Path(cache.absolutePath)).use { client ->
    ...
    fun cancelledPluginRefreshDoesNotSurfaceCachedDataAsAnErrorResult(): Unit = runBlocking {
        ...
        CodexAgentClient({ runtime }, requestTimeoutMillis = 5_000, pluginCacheDirectory = Path(cache.absolutePath)).use { client ->
