...
class AppServerConnectionTest {
    @Test
    fun `initializes once and executes a typed request`() = runBlocking {
    ...
    @Test
    fun `decodes server events and writes typed responses`() = runBlocking {
        val runtime = FakeRuntime()
        val connection = connection(runtime)
        initialize(connection, runtime)
        ...
        runtime.receive(
            """{"id":77,"method":"item/tool/call","params":{"threadId":"t","turnId":"u","callId":"c","tool":"documents_read","arguments":{}}}""",
        )
    ...
    @Test
    fun `malformed output fails pending work and reports a typed failure`() = runBlocking {
        val runtime = FakeRuntime()
        val connection = connection(runtime)
        initialize(connection, runtime)
    ...
    @Test
    fun `full event buffer fails explicitly without blocking the runtime reader`() = runBlocking {
        val runtime = FakeRuntime()
